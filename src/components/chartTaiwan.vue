...
// import 'https://unpkg.com/topojson@3'
// import usePie from './usePie.js'
export default {
  name: 'Chart_2',
  setup () {
    // const taiwanCountry = reactive([])
    // const draw = (mapData) => {
    ...
    onMounted(() => {
      ...
      array.push({ city_name: "基隆", product: "鼎邊趖、甜不辣、叉包" });

      array.push({
        city_name: "台北",
        product:
          "八里黑糯米、關渡紅仁鹹蛋、淡水鐵蛋、白沙灣海砂土豆、郭元益綠豆糕、金山跳石芋、鶯歌陶瓷、士林刀、陽明山草山柑仔、石門鐵觀音、文山包種茶、木柵鐵觀音、士林夜市大餅包小餅、九份芋圓…等"
      });
      array.push({ city_name: "桃園", product: "大溪豆柑、龍潭龍泉茶、石門活魚…等" });
      array.push({
        city_name: "新竹",
        product:
          "貢丸、米粉、花生醬、香粉、新埔柿餅、文旦柚茶、凸柑、玻璃製品、燈籠…等"
      });
      array.push({
        city_name: "苗栗",
        product:
          "柑風茶、酸柑茶、文旦柚茶、菊花茶、公館福菜、大湖草莓、大湖蠶絲、三義木雕、三灣梨、大湖紅芭樂、卓蘭椪柑、公館紫蘇梅、牛心、水柿…等"
      });
      array.push({
        city_name: "台中",
        product:
          "太陽餅、一心豆干、蘇州采芝齋月餅、鳳梨酥、酥餅、一口香、豐原雪花餅、沙鹿石頭肉角、員林蜜餞、大甲草蓆、帽、東勢椪柑、巨峰葡萄、密豆冰…等"
      });
      array.push({
        city_name: "彰化",
        product:
          "北斗肉園、北斗酥糖、楓坑米粉、溪洲酸菜、美華小月餅、二水螺溪硯、鹿港牛舌餅、豬油荖、糖蔥、麥芽酥、鳳眼糕、口酥餅、杏仁荖、土豆荖、綠豆糕、彩頭酥、燈籠、木器、錫器、紙扇、佛像雕刻、花卉…等"
      });

      array.push({
        city_name: "南投",
        product:
          "凍頂烏龍茶、松柏長青茶、鶴岡紅茶、雙冬檳榔、竹山紅蕃薯、竹山筍乾、苦茶油、集集芋頭、竹山竹器製品、埔里甘蔗、甘蔗筍、紹興酒、米粉、香菇、木耳、茭白筍、紅茶、花卉、蝴蝶標本…等"
      });

      array.push({
        city_name: "雲林",
        product:
          "古坑麻竹筍、草嶺苦茶油、斗六文旦、莿桐蒜油精、北港瓜子、花生、花生油、蠶豆、杏仁粉、麻油、凸餅、新港飴、花生酥、花生醬、西螺濁水溪米、醬油、西瓜、豆腐乳、蕃薯粉、醬菜…等"
      });

      array.push({
        city_name: "嘉義",
        product:
          "新港飴、方塊酥、中埔葡萄柚、梅山檳榔、大埔麻竹筍、布袋菜脯、阿里山山葵、梅山愛玉子、酸梅…等"
      });

      array.push({
        city_name: "台南",
        product:
          "麻豆酪梨、文旦、玉井芒果、關廟鳳梨、鹹鳳梨、白河蓮子、藕粉、官田菱角、東山福肉、龍眼、柳丁、左鎮破布子、黑橋牌香腸、棺材板、古月餅、擔仔麵、富香齋糕皮冬瓜肉餅、安平蜜餞、關廟藤竹器…等"
      });

      array.push({
        city_name: "高雄",
        product: "鳳班綠豆椪、岡山豆瓣醬、醬菜、蜂蜜、旗山集集蕉園、甲仙芋頭、海產…等"
      });

      array.push({
        city_name: "屏東",
        product:
          "港口茶、恆春檳榔、林邊蓮霧、潮州八爺酸菜、椰子、恆春瓊麻、峰港洋蔥、美濃傘…等"
      });

      array.push({
        city_name: "宜蘭",
        product: "鴨賞、膽肝、羊羹、金棗、李仔糕、金棗糕、蜜餞、南方澳珊瑚、海產…等"
      });

      array.push({
        city_name: "花蓮",
        product:
          "栗餅、花蓮薯、蕃薯糖、栗米花、米糖、玉里羊羹、天鶴茶、文旦柚茶、舞鶴咖啡、大理石、玫瑰石、金瓜石、雞血石、白雲石、海洋化石…等"
      });

      array.push({
        city_name: "台東",
        product:
          "	甘薯、小米、洛神花、太麻里金針、香菇、成功柴魚、柿餅、福鹿茶、菊花茶…等"
      });

      array.push({
        city_name: "澎湖",
        product:
          "	哈密瓜、絲瓜、嘉寶瓜、紫菜、海帶、龍蝦、花生、花生酥、海苔酥、蝦仁酥、鹹餅、海產乾貨、竹笠、文石、貓公石、珊瑚、海樹、貝殼、珍珠…等"
      })

      // $("path").hover(function () {
      //   console.log($(this).attr("name"));
      //   for (var i = 0; i < array.length; i++) {
      //     if ($(this).attr("name") == array[i].city_name) {
      //       $("#h2").html(array[i].city_name);
      //       $("#h3").html(array[i].product);
      //     }
      //   }
      // });
    })
...
path {
  fill: #96F550;
  filter: drop-shadow( -2.5px -2.5px 6px #8E8DBE);
  cursor: grab;
  &:hover {
    fill: #7A306C;
    // filter: drop-shadow( 10px 10px 3px #8E8DBE);
    // transition: 0.5s;
